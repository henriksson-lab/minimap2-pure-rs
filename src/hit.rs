...
use crate::types::{AlignReg, Mm128, PARENT_TMP_PRI, PARENT_UNSET};
...
fn reg_set_coor(r: &mut AlignReg, qlen: i32, a: &[Mm128], is_qstrand: bool) {
    ...
    r.rs = if (a[k].x as i32) + 1 > q_span {
        (a[k].x as i32) + 1 - q_span
    } else {
        0
    };
...
pub fn split_reg(
    ...
    if (r2.as_ as usize + r2.cnt as usize) <= a.len() {
        reg_set_coor(&mut r2, qlen, a, is_qstrand);
    }
        ...
        for &j in &w {
                ...
                && uncov_len <= mask_len
            {
                ...
                if j_has_extra && i_has_extra {
                    let j_rid = regs[j].rid;
                    let j_rs = regs[j].rs;
                    let j_re = regs[j].re;
                    let i_rid = regs[i].rid;
                    let i_rs = regs[i].rs;
                    let i_re = regs[i].re;
...
pub fn hit_sort(regs: &mut Vec<AlignReg>, alt_diff_frac: f32) {
    ...
    for (i, r) in regs.iter().enumerate() {
        if r.inv || r.cnt > 0 {
            ...
            let score = if r.is_alt {
                alt_score(score, alt_diff_frac)
            } else {
                score
            };
...
pub fn update_dp_max(qlen: i32, regs: &mut [AlignReg], frac: f32, a: i32, b: i32) {
    ...
    for r in regs.iter_mut() {
        if let Some(ref mut p) = r.extra {
            ...
            let new_max =
                (a as f64 * (r.mlen as f64 - b2 * n_mis as f64 - gap_cost) + 0.499) as i32;
...
pub fn select_sub(pri_ratio: f32, min_diff: i32, best_n: i32, regs: &mut Vec<AlignReg>) {
    ...
    for i in 0..n {
        ...
        {
            // Not identical hits
            if !(regs[i].qs == regs[p].qs
                && regs[i].qe == regs[p].qe
                && regs[i].rid == regs[p].rid
                && regs[i].rs == regs[p].rs
                && regs[i].re == regs[p].re)
            {
                to_keep = true;
...
pub fn set_mapq(
    ...
    for r in regs.iter() {
        if r.parent == r.id {
            sum_sc += r.score as i64;
        } else if r.is_spliced {
            n_2nd_splice += 1;
        }
    }
    ...
    for r in regs.iter_mut() {
        if r.inv {
            ...
            let pen_s1 = (if r.score > 100 {
                1.0
            } else {
                0.01 * r.score as f32
            }) * uniq_ratio;
            let pen_cm = if r.cnt > 10 {
                1.0f32
            } else {
                0.1 * r.cnt as f32
            };
            let pen_cm = pen_s1.min(pen_cm);
            let subsc = if r.subsc > min_chain_sc {
                r.subsc
            } else {
                min_chain_sc
            };
            ...
            let mapq = if let Some(ref p) = r.extra {
                if p.dp_max2 > 0 && p.dp_max > 0 {
                    ...
                    let mut mq = (identity
                        ...
                        * Q_COEF
                        * (1.0 - x * x)
                        * (p.dp_max as f32 / match_sc as f32).ln())
                        as i32;
                    if !is_sr {
                        let mq_alt = (6.02 * identity * identity * (p.dp_max - p.dp_max2) as f32
                            / match_sc as f32
                            + 0.499) as i32;
                        mq = mq.min(mq_alt);
                    }
...
mod tests {
    ...
    fn test_set_parent_two_overlapping() {
        ...
        for i in 0..5 {
            a.push(Mm128::new(
                0u64 << 32 | (i * 50 + 50) as u64,
                15u64 << 32 | (i * 50 + 50) as u64,
            ));
        }
        for i in 0..3 {
            a.push(Mm128::new(
                0u64 << 32 | (i * 50 + 75) as u64,
                15u64 << 32 | (i * 50 + 75) as u64,
            ));
        }
