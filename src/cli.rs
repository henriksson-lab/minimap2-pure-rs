...
use crate::index::{self, MmIdx};
use crate::options::{self, IdxOpt, MapOpt};
use crate::pipeline;
use crate::types::MM_VERSION;
use clap::Parser;
    ...
    T: Into<std::ffi::OsString> + Clone,
{
    ...
    let argv: Vec<std::ffi::OsString> = args_in.into_iter().map(Into::into).collect();
    ...
    if cli.hpc {
        io.flag |= crate::flags::IdxFlags::HPC;
    }
    if cli.idx_no_seq {
        io.flag |= crate::flags::IdxFlags::NO_SEQ;
    }
    if cli.write_junc {
    ...
    if let Some(ref junc_path) = cli.junc_bed {
        match crate::junc::read_junc_bed(&mut mi, junc_path) {
    ...
    if let Some(ref jump_path) = cli.jump_bed {
        match crate::junc::read_jump_bed(&mut mi, jump_path, crate::junc::JUNC_ANNO, -1) {
    ...
    if let Some(ref pass1_path) = cli.pass1_bed {
        match crate::junc::read_jump_bed(&mut mi, pass1_path, crate::junc::JUNC_MISC, 5) {
            Ok(n) => eprintln!(
                "[M::main] loaded {} pass-1 jump edges from {}",
                n, pass1_path
            ),
    ...
    crate::align::report_align_profile();
    0
}
